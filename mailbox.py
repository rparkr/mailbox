...
def _(mo):
    ...
    _about = mo.accordion(
        {
            "## About this app": (
                f"""This is a simple application I made for teaching my son about letters. Typical alphabet games and books use _starts with_ for letter association, like "**A** is for **A**pple :apple:". But I have observed that children often learn letters based on _prominence_: how prominently that letter is used within a word, like the **a** in pl**a**y or the **f**s in **f**lu**ff**y. And other times it is valuable to recognize that letters all throughout a word are important. When we learn to speed read, we are not just looking at the first letter of a word, but rather the word as a whole, thus, all the letters are important in that word.
